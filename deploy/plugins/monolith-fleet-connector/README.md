...
- `enableRepairActions`: defaults to `false`; enables adapter-link repair calls
  only when a board user supplies an approval/change-request reference.
- `enableRoutineRepairActions`: defaults to `false`; enables creating
  approval-bound Fleet Manager routine-repair proposals. Direct routine repair
...
- `enableCostSyncActions`: defaults to `false`; enables non-dry-run cost sync.
  Apply still requires an approval/change-request reference.
...
- `scheduledCostSyncApply`: defaults to `false`; keeps scheduled cost sync in
  dry-run mode unless explicitly enabled. Scheduled runs do not carry an
  operator approval reference, so keep the pilot in dry-run unless an
  approval-bearing scheduled path is added.
...
- API routes:
  ...
  - `POST /fleet/sync-costs` (board-only)
  - `POST /fleet/lifecycle` (board-only)
  - `POST /fleet/managed-resources/reconcile` (board-only)
- Managed resources: the manifest declares a paused Fleet Governance Operator,
...
- Scheduled job: `scheduled-cost-sync` runs hourly. It is disabled by default
  and dry-runs by default when enabled.

## Safety Model

The connector never stores a Fleet token in plugin state and redacts secret
configuration from data responses. Mutating API routes are board-only, and
mutating actions record Paperclip activity plus metrics. Every non-dry-run
mutation requires an approval/change-request reference. Cost sync is dry-run by
default at the action/API boundary; applying cost events requires
`enableCostSyncActions: true`, `dryRun: false`, and an approval/change-request
reference. Scheduled cost sync has a second guard: the hourly job skips unless
`enableScheduledCostSync` is true, and the pilot should remain dry-run because
scheduled runs do not include an operator approval reference.
...
Routine repair is also governed. Dry-run calls are safe by default and write
...
flags and approval/change-request references above.
Register and adapter-link repair actions are disabled by default and require an
approval/change-request reference when explicitly enabled.
